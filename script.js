...
function validateForm() {
    ...
    if (isValid) {
        ...
        // document.querySelector('.vdp-frm-btn.submit').style.display = 'block'; 
    } else {
        if (firstErrorField) {
...
function calculateRisk() {
    ...
    inputs.forEach(input => {
        ...
        heartRiskInputedData.push(`${questionLabel}: ${option}`)
    });
    console.log(heartRiskInputedData)
    ...
    const riskLevel = totalPoints >= 72 ? 'Your heart disease risk is very high. You should talk to your healthcare professional as soon as possible and discuss reducing your risk.' : totalPoints >= 58 && totalPoints <= 70 ? 'Your heart disease risk is high. You need to work with your healthcare professional on a plan to reduce your risk.' : totalPoints >= 34 && totalPoints <= 56 ? 'Your heart disease risk is average. Talk to your healthcare professional about ways you can lower your risk.' : totalPoints <= 32 ? 'Your heart disease risk is low. Keep up the good work.' : '';
    resultDiv.innerHTML = `
...
document.querySelector('button[type="reset"]').addEventListener('click', () => {
    resetValidationState(); // Reset validation states
});

// close display outputf and reset form
function closeOutput () {
    ...
    // document.getElementById('resetFormButton').addEventListener('click', function() {
        ...
        // Clear all error messages and remove error classes
        form.style.display = 'block'; 
        document.getElementById('vdp-display-output-container').style.display = 'none'; 
        // formGroups.forEach(group => {
        //     group.classList.remove('error');
        //     const errorMessage = group.querySelector('.vdp-error-message');
        //     if (errorMessage) {
...
// Function to close the display output and reset the form
function closeOutput() {
    resetValidationState();
    form.reset();
    form.style.display = 'block'; 
